...
No HTML nós temos dois tipos de listas, sendo elas `listas não ordenadas` e `listas ordenadas`.
...
Por padrão os itens das listas ordenadas serão marcados por números, mas nós podemos mudar utilizando o `type="valor"` dentro da tag `<ul>`.
